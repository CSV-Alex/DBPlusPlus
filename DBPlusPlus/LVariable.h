...
// -----------------------------------------------------------------------------
// Función: get_n_attributes
// Objetivo de la función:
//     Obtener el número de atributos definidos para una relación dada,
//     leyendo su línea en el archivo de esquema.
// Input:
//     const char* relacion  – nombre de la relación a buscar en el esquema.
// Output:
//     int  – número de atributos (pares offset|size), lanzando excepción
//            si no puede abrir el esquema o no encuentra la relación.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: get_nth_attribute
// Objetivo de la función:
//     Determinar la posición (índice 1-based) de un atributo dentro de la definición
//     de una relación en el esquema.
// Input:
//     const char relacion[]  – nombre de la relación.
//     const char atributo[]  – nombre del atributo a buscar.
// Output:
//     int  – número de orden del atributo (comenzando en 1), o -1 si no existe
//            o hay error al abrir el esquema.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: read_variable_register
// Objetivo de la función:
//     Desempaquetar un registro variable (binario) y devolver sus campos
//     concatenados en una cadena con ‘#’ como separador.
// Input:
//     const char registro_variable[]  – buffer con header de offsets|sizes| y datos.
//     const char relacion[]           – nombre de la relación para calcular offsets.
// Output:
//     std::string  – campos del registro unidos por ‘#’, lanza excepción si
//                    número de atributos inválido o rango fuera de límites.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: format_registro_variable
// Objetivo de la función:
//     Formatear una cadena de campos (‘campo1#campo2#…’) en un registro variable
//     binario con encabezado de offsets y tamaños.
// Input:
//     const char registro[]   – cadena con valores separados por ‘#’.
//     const char relacion[]   – nombre de la relación para obtener metadatos.
// Output:
//     char*  – buffer dinámico (nuevo) conteniendo header y datos; debe liberarse
//              (delete[]) tras su uso.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: actualizarDirBloquesVariable
// Objetivo de la función:
//     Actualizar en dirBloques.txt la entrada de un bloque variable para reflejar
//     los cambios en espacio libre tras inserciones/eliminaciones.
// Input:
//     int bloqueN  – número de bloque a actualizar.
//     Disco& disco – referencia al objeto que provee parámetros y métodos de volcado.
// Output:
//     bool  – true si la actualización se escribó correctamente, false en caso de error.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: insert_registro_variable
// Objetivo de la función:
//     Insertar en memoria un registro variable en un bloque, actualizando la
//     metadata interna y el espacio libre.
// Input:
//     const char registro_variable[] – buffer binario del registro a insertar.
//     int& espacio_libre_bloque       – referencia para devolver el nuevo espacio libre.
//     char* bloque_variable           – buffer completo del bloque en memoria.
//     Disco& disco                    – objeto que provee tamaño de bloque y volcado.
// Output:
//     int  – 1 si éxito, 0 si no cabe (write_pos < 0), 2 si espacio interno insuficiente.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: eliminar_registro_variable
// Objetivo de la función:
//     Eliminar un registro de un bloque variable, compactar los datos,
//     recalcular metadata y actualizar el directorio de bloques.
// Input:
//     int bloqueN    – número de bloque de donde eliminar.
//     int r_index    – índice 1-based del registro a borrar.
//     int tamBloque  – tamaño en bytes del bloque.
//     Disco& disco   – objeto para actualizar directorio y volcar sectores.
// Output:
//     bool  – true si la eliminación y actualizaciones fueron exitosas, false en error.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: modify_registro_variable
// Objetivo de la función:
//     Modificar el valor de un atributo en un registro variable dentro de un bloque,
//     eliminando el viejo, actualizando dirBloques y reinserción del registro nuevo.
// Input:
//     int NBloque               – número de bloque a modificar.
//     int r_index               – índice 1-based del registro.
//     const char relacion[]     – nombre de la relación (esquema).
//     const char atributo[]     – nombre del atributo a cambiar.
//     const char new_value[]    – nuevo valor a asignar.
//     Disco& disco              – objeto para manejos de bloque y directorio.
// Output:
//     bool  – true si la modificación tuvo éxito, false en caso de cualquier fallo.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
//AUXILIARES
// -----------------------------------------------------------------------------
// Función: adicionarRegistroUnicoVariable
// Objetivo de la función:
//     Formatear un registro de texto a binario variable e insertarlo en Bloque1,
//     además de volcar y actualizar dirBloques.
// Input:
//     const char* registroTxt – línea de texto (‘campo1#…\n’).
//     const char* relacion    – nombre de la relación para formateo.
//     Disco& disco            – objeto que maneja bloque y volcado.
// Output:
//     bool  – true si se insertó y actualizó correctamente, false de lo contrario.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: volcar_bloque_a_archivo
// Objetivo de la función:
//     Volcar el contenido no nulo de un bloque variable a un archivo de texto,
//     eliminando bytes ‘\0’ finales.
// Input:
//     const char* bloque_variable – buffer del bloque en memoria.
//     int tamBloque               – tamaño total del bloque.
// Output:
//     bool  – true si escribe el archivo sin errores, false si falla operación I/O.
// Autor: Alexander
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Función: adicionarNRegistrosVariable
// Objetivo de la función:
//     Leer n líneas de un archivo de texto (omitiendo cabecera) y añadirlas como
//     registros variables en Bloque1, volcando y actualizando directorio.
// Input:
//     int n                   – número de registros a leer e insertar.
//     const char* txtPath     – ruta del archivo de texto con registros.
//     const char* relacion    – nombre de la relación para formateo.
//     Disco& disco            – objeto para manejo de bloque y directorio.
// Output:
//     bool  – true si todos los registros se añaden y actualizan correctamente.
// Autor: Alexander
// -----------------------------------------------------------------------------
