...
std::string parseQueryCondition(std::string query);  

void convertTsvToTxt(const std::string& tsvFile, const std::string& txtFile);  
void duplicatesLines(const std::string& schema);  

std::string getRelationR(std::string schema, std::string input);  

void duplicatesLines(const string& schema);

vector<string> getFields(string input, string schema, string relation);  
